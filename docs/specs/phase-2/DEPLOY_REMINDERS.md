...
| Item | When | Action |
|------|------|--------|
| **`OPENAI_API_KEY`** | When Reflect should use live LLM | Enables OpenAI-backed Wisewave responses on `/reflect` and `/api/wisewave/sessions`. Optional `OPENAI_MODEL` (default `gpt-5.5`). Without the key, responses stay rule-based. |
