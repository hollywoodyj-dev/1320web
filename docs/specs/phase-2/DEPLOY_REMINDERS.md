...
| **`POSTGRES_URL`** | All Phase 2 domain features | Run `npm run db:migrate` after schema changes. On Vercel/serverless use a **pooled** Postgres URL (Neon: host with `-pooler`) to avoid `too many connections` on login/API routes. |
