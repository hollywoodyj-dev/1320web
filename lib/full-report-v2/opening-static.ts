/** Fixed copy for Page 01 — Opening Note + Disclaimer (spec v1.0) */

export const OPENING_HOW_TO_USE = [
  {
    icon: "◉",
    title: "Read Slowly",
    description:
      "Give yourself space to receive each section without rushing to define yourself.",
  },
  {
    icon: "☉",
    title: "Use as a Mirror",
    description:
      "This is not about labels. It is about recognition, reflection, and conscious choice.",
  },
  {
    icon: "☷",
    title: "Journal & Reflect",
    description: "Write freely. Let insights emerge in your own words and timing.",
  },
  {
    icon: "✶",
    title: "Apply with Awareness",
    description:
      "Take what resonates. Leave what does not. Integration is a living process.",
  },
  {
    icon: "♡",
    title: "Return Often",
    description:
      "Revisit this report as you grow. You evolve, and so will your reflections.",
  },
] as const;

export const OPENING_GUIDANCE_ROWS = [
  {
    icon: "◌",
    title: "This is a Mirror,\nNot a Sentence",
    copy: "You are not your code. Your code is a reflection of patterns, not a definition of your destiny.",
  },
  {
    icon: "✦",
    title: "You Have\nFree Will",
    copy: "You always have the freedom to choose, to grow, to change, and to create.",
  },
  {
    icon: "♧",
    title: "Growth is\nNon-Linear",
    copy: "Insights may reveal layers over time. There is no “right pace” — trust your inner timing.",
  },
  {
    icon: "◌",
    title: "Not Everything\nWill Resonate",
    copy: "Take what is true for you now. Leave what is not. Integration happens naturally.",
  },
  {
    icon: "♡",
    title: "You Are\nAlready Whole",
    copy: "This report points to potential, not lack. You are not incomplete. You are already whole.",
  },
] as const;

export const OPENING_NOT_LIST = [
  { icon: "◌", text: "Not a prediction of future events" },
  { icon: "✚", text: "Not medical or psychological advice or diagnosis" },
  { icon: "☁", text: "Not a substitute for therapy or professional support" },
  { icon: "$", text: "Not financial, tax, business, or investment advice" },
  { icon: "⚖", text: "Not legal advice or decision-making authority" },
  { icon: "✧", text: "Not superstition, fortune-telling, or fixed destiny" },
] as const;

export const OPENING_FOOTER_ITEMS = [
  {
    icon: "◇",
    content:
      "Use this report as a mirror for awareness, not as a fixed identity or absolute truth.",
  },
  {
    icon: "✦",
    content: "You are more than any pattern. You are consciousness in motion.",
  },
  {
    icon: "♡",
    content: "Thank you for choosing self-awareness, discernment, and integration.",
  },
] as const;
